...
export async function fetchLatLonByCity(city: string, countryCode?: string) {
  try {
    const q = countryCode ? `${city},${countryCode}` : city
    const res = await fetch(`https://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(q)}&limit=1&appid=${process.env.OPENWEATHER_API_KEY}`)
