...
describe("HourlyCard", () => {
  it("renders one column per hour with time labels and the default metric", () => {
    ...
    render(<HourlyCard {...baseCardProps({ options: o })} options={o} />);
  ...
  it("switches metric when a chip is pressed", () => {
    render(<HourlyCard {...baseCardProps({ options: opts })} options={opts} />);
  ...
  it("shows a skeleton while loading", () => {
    render(<HourlyCard {...baseCardProps({ status: "loading", snapshot: undefined })} options={opts} />);
